...
RUN mkdir -p /root/quartus && \
    ...
    /root/quartus/components/QuartusLiteSetup-20.1.0.711-linux.run --mode unattended --installdir /opt/intelFPGA_lite/20.1 --accept_eula 1 && \
    rm -rf /root/quartus && rm -rf /root/setup*


RUN echo 'alias quartus="/opt/intelFPGA_lite/20.1/quartus/bin/quartus"'>> ~/.bashrc